...
function App() {
  ...
  const [input, setInput] = useState('Type in the prompt...');
    ...
    if (input.trim()) {
      ...
      try {
        ...
        const response = await axios.post(
          'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=Enter your key',
          {
            "contents": [
              {
                "parts": [
